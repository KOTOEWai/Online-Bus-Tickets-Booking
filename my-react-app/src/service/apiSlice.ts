...
import type {
  BusDetails, Notification, ContactFormData,
...
export const busApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    searchBuses: builder.query<BusDetails[], SearchPayload>({
      query: (payload) => ({
        url: 'bus/search.php', // API endpoint
    ...
    destination: builder.query<DestinationDetail, string>({
      query: (id) => (
        {
          url: `bus/destinations.php?id=${id}`,
          method: 'GET',
    ...
    getNotification: builder.query<Notification[], { user_id: string }>({
      query: ({ user_id }) => ({
        url: `user/notifications.php?user_id=${user_id}`,
        method: 'GET',
    ...
    markNotificationAsRead: builder.mutation<any, { notification_id: number; user_id: string }>({
      query: ({ notification_id, user_id }) => ({
        url: 'user/read_notification.php',
        method: 'POST',
    ...
    getUserProfile: builder.query<UserInfo, string>({
      query: (userId) => ({
        url: `user/info.php?user_id=${userId}`,
        method: 'GET',
    ...
    getScheduleSeats: builder.query<any, { id: string }>({
      query: ({ id }) => ({
        url: `booking/seats.php?schedule_id=${id}`,
        method: 'GET',
      }),
    }),
    bookSeats: builder.mutation<any, { id: string; selected_seats: number[] }>({
      query: (body) => ({
        url: 'booking/book.php',
        method: 'POST',
        body,
      }),
    }),
    getBookingInfo: builder.query<BookingInfo, string>({
      query: (bookingId) => `booking/info.php?booking_id=${bookingId}`,
